...
public static void hapusData(formatBiodata biodataMahasiswa[]){
	...
	if(biodataMahasiswa[i].nama.equalsIgnoreCase(nama)){
		...
		if(dataDiTemukan){
			...
			for (int i = dataDiHapus; i < N - 1; i++) {
		biodataMahasiswa[i] = biodataMahasiswa[i + 1];
			}
			 biodataMahasiswa[N - 1] = null;
			 N--;
			 System.out.println("Data dengan nama '" + nama + "' telah dihapus.");
		}else {
			System.out.println("Maaf, data dengan nama '" + nama + "' tidak ditemukan.");
        }
